 ...
 */
class UsersInventorySearch extends Users
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        $query = Users::find()->where("users.id !=" . Yii::$app->user->id . " AND role_id != " . Yii::$app->params['super_admin_role_id']);
        $requestParam = $_GET;
        $query->joinWith(['inventoryProducts'=>function( $q )use( $requestParam ) {
                $q->andWhere( "inventory_products.user_id = users.id");
                return $q;
            }], false , 'LEFT JOIN' );
